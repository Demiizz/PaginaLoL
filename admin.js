...
function renderSteps() {
  const steps = [
    { label: "Equipos", done: data.teams.length === TEAM_COUNT },
...
function updateTeamCount() {
  ...
  count.textContent = t.length + " / " + TEAM_COUNT + " equipos cargados";
  count.className = "count-line " + (t.length === TEAM_COUNT ? "ok" : "bad");
  el("drawStartBtn").disabled = t.length !== TEAM_COUNT;
}
teamsInput.addEventListener("input", updateTeamCount);

el("drawStartBtn").addEventListener("click", () => {
  const teams = currentTeamsFromInput();
  if (teams.length !== TEAM_COUNT) return;
  ...
  el("drawStatus").textContent = "Quedan " + TEAM_COUNT + " equipos por sortear.";
  renderDrawSlots();
  renderSteps();
});
...
el("revealBtn").addEventListener("click", () => {
  ...
  Draw.revealNext({
    ...
    onLocked(group, slotIdx, name) {
      ...
      el("revealBtn").disabled = false;
      const remaining = TEAM_COUNT - Draw.revealIndex;
...
function bootFromData() {
  teamsInput.value = data.teams.length === TEAM_COUNT ? data.teams.join("\n") : DEFAULT_TEAMS.join("\n");
  renderCloudPanel();
  updateTeamCount();
  renderSteps();
