/* =========================================================
   ...
   ---------------------------------------------------------
   Mantiene EXACTAMENTE la lógica original: la cantidad de
   equipos se define en TEAM_COUNT (config.js) y se reparte
   en dos grupos iguales. Revelación una por una alternando A/B, con
...
const Draw = {
  ...
  /** Arranca un sorteo nuevo con los equipos cargados (se reparten mitad y mitad) */
  init(teams) {
    this.teams = teams;
    this.drawOrder = shuffle(teams);
    this.revealIndex = 0;
    const groupSize = teams.length / 2;
    this.slots = { A: Array(groupSize).fill(null), B: Array(groupSize).fill(null) };
